...
## 시맨틱·성능 점검 기준 (2026-08-21)

**결정**: 전체 `src`를 정적 점검하되, 마크업을 일괄 교체하지 않고 실제 동작·접근성·렌더링 비용에 영향이 확인된 부분만 수정한다.

- 요청 메시지·예산 입력은 label, id, 오류 설명을 연결해 보조기술이 입력 목적과 검증 오류를 읽을 수 있게 했다.
- 제품 상세의 breadcrumb를 목록 구조로 바꾸고, 키보드 menu 동작을 구현하지 않은 버튼 묶음에서 `menu`/`menuitem` ARIA 역할을 제거했다. 잘못된 ARIA보다 네이티브 button 의미가 정확하다.
- 중첩된 main 랜드마크를 제거하고, listbox가 아닌 수량 선택 UI의 `aria-haspopup="listbox"`를 제거했다.
- 상품 카드 원격 이미지를 Next Image로 전환하고, S3 다단계 서브도메인을 허용하는 `remotePatterns`로 바꿨다. 상품 등록 이력의 반복 카테고리 탐색은 Map 조회로 바꿨다.

**의도적으로 보류한 범위**: 데스크톱 표 레이아웃 전체를 한 번에 table로 재작성하지 않았다. 모바일 카드 UI와 링크·행 액션 구조가 얽혀 있어 시각 회귀 위험이 크므로, 실제 화면 비교가 가능한 별도 작업으로 나눈다.
