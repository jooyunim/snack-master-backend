...
**결정**: 별도 Meilisearch 인스턴스는 도입하지 않고, RDS PostgreSQL의 `pg_trgm` 확장과 상품명 GIN 인덱스를 사용한다. (비용상의 문제가 아니었으면 ElasticSearch나 Meilisearch를 썼을것이나, 메모리 차지가 크기 때문에 둘 다 t3.micro요금제 사용중인 상태로서는 사용할 수 없었다.)
...
## 상품 한글 초성·자모 검색 추가 (2026-08-24)

**결정**: 별도 검색 엔진이나 운영자 입력 필드 없이, 상품명에서 `searchInitials`와 `searchJamo`를 서버가 자동 생성해 PostgreSQL `pg_trgm` 인덱스로 검색한다.

**이 방식을 고르게 된 추가 설명**:
- 현재 EC2 t3.micro의 메모리·디스크 여유가 적어 Meilisearch를 같은 서버에 두는 것은 위험하다.
- 초성·자모는 pg_trgm만으로 자동 해석되지 않기에, 검색용 문자열을 자동 생성하는 방식이 필요하다.
- 별도 DB나 동기화 파이프라인 없이 현재 RDS와 Prisma 구조에 맞춰 구현할 수 있다.
- 운영자 입력 부담 없이 기존 상품까지 자동 적용된다.

**근거**:

- `pg_trgm`은 한글 음절과 호환 자모를 자동으로 같은 문자열로 해석하지 않는다. 따라서 `모나미`와 `모남ㅣ` 같은 혼합 입력은 일반 부분 검색이나 유사도 검색만으로 일치하지 않는다.
- 상품 등록·수정 서비스 한 곳에서 검색값을 만들면 모든 신규·수정 상품이 동기화되며, 운영자가 초성 또는 자모를 직접 관리할 필요가 없다.
- migration은 빈 문자열 기본값과 부분 GIN trigram 인덱스만 추가한다. 배포 후 `after_install.sh`가 빈 검색값 상품을 100개씩 갱신해 기존 데이터도 검색에서 누락되지 않게 한다.
- 기존 `GET /products`의 회사·삭제 여부·카테고리 필터와 cursor 정렬은 유지하고, 이름·초성·자모 조건만 `OR`로 추가했다. 이로써 회사 간 검색 결과 노출 범위는 변하지 않는다.
...
**의도적으로 제외한 범위**: `모나비 → 모나미` 같은 오타 유사도 순위와 영문 키보드 상태 변환은 별도 랭킹·오탐 기준 조정이 필요하므로 이번 변경에 섞지 않는다. 현재 `pg_trgm` 인덱스는 다음 단계의 유사도 검색 기반으로 유지한다.

---

## FE `/products` SSR ��ȯ ��� (2026-08-25)

`/products` �������� CSR���� SSR�� ��ȯ�ߴ�. ù ���� �� �������� `GET /categories`�� `GET /products`�� ���� ȣ���ؼ� ù ȭ���� �׸����� �����, ���� ������/�˻�/������ ���� Ŭ���̾�Ʈ ��ȣ�ۿ��� �����ߴ�. �α��� ����� ���� `isWished` ���� ���� ������ ��߳��� �ʵ��� ��û ��Ű�� ���� fetch�� �Բ� �Ǿ���.

������ ��ǰ ����Ʈ ������������ �����ߴ�. �ٸ� �������� �ǵ帮�� �ʾҰ�, Ŀ���� ������ �ʾҴ�.

Ŀ�� �޽��� �ĺ�: `feat(products): convert product list page to SSR`
